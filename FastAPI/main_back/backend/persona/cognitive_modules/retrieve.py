...
def retrieve(persona, perceived): 
  '''
  [신누리]
  Persona 객체, ConceptNode 객체리스트 를 받아서, 
  Persona의 associate_memory 에서 ConceptNode의 subject, predicate, object 를 키워드의 키로
  curr_event 현재 event와, 관련 events(ConceptNode), 관련 thoughts(ConceptNode)를 포함해 반환함

  [INPUT]
  persona : 현재 Persona 객체
  perceived : [ConceptNode,...] ConceptNode 객체 리스트
  ...
  각 event의 subject, predicate, object를 key로 사용하여, 
  이전 기억([ConceptNode...]) 에서의 관련성있는 events, thoughts 를 가져옴
  '''
...
def normalize_dict_floats(d, target_min, target_max):
  '''
  [신누리]
  딕셔너리의 크기를 일정 범위로 일반화하는 함수.

  [INPUT]
  d : 크기를 저장하고있는 딕셔너리
  target_min : 일반화하기위한 아랫 값
  target_max : 일반화하기위한 윗 값
  
  [OUTPUT]
  아랫값 target_min 과 윗값 target_max로 일반화한 딕셔너리
  -----
  Example input: 
...
def top_highest_x_values(d, x):
  '''
  [신누리]
  딕셔너리에서 크기를 내림차순으로 정렬 후 x개를 반환하는 함수.
...
def extract_recency(persona, nodes):
  '''
  [신누리]
  nodes 를 최근 순서대로 점수화하여 반환.
  [INPUT]
  persona : 현재 Persona 객체
  nodes : [ConceptNode,...] ConceptNode 객체 리스트
  [OUTPUT]
  {
    "node_816" : 0.995**1,
    "node_815" : 0.995**2,
...
def extract_importance(persona, nodes):
  '''
  [신누리]
  현재 Persona의 기억을 중요도를 포함해서 반환하는 함수.
...
def extract_relevance(persona, nodes, focal_pt): 
  '''
  [신누리]
  현재 Persona의 기억과 이전기억의 각 임베딩 값으로 코사인 유사도를 포함해서 반환하는 함수.
...
def new_retrieve(persona, focal_points, n_count=30): 
  '''
  [신누리]
  현재 Persona의 사건과 이전 기억의 관련성을 기반으로 n_count개의 기억을 반환하는 함수.

  [INPUT]
  persona : 현재 Persona 객체
  focal_points : ['monk meet nun', 'bed is idle'...] 같은 문자열 리스트, description같은 값으로 추정됨.
  n_count : 관련기억을 반환받을 갯수

  [OUTPUT]
  [ConceptNode,...] 관련기억을 리스트 형식으로 반환합니다.
  '''
  """
  ...
  for focal_pt in focal_points: 
    ...
    '''
    [신누리]
    embedding_key에 'idle'이 포함되지 않는 사건을 가져와서 [[created(날짜), Conceptnode(기억)]...]으로 저장 후,
    created(날짜) 순으로 정렬하여 nodes에 저장을 하는 부분.
    '''
    ...
    '''
    [신누리]
    최신 점수(최근일 수록 점수가 높음), 중요도 점수, 유사도 점수를 일반화하여 저장하는 부분.
    '''
    ...
    '''
    [신누리]
    gw는 최신 점수(최근일 수록 점수가 높음), 중요도 점수, 유사도 점수의 가중치를 설정 합니다.
    현재 [0.5, 3, 2] 로 되어있으나, 가중치를 경험적으로 수정하는 방법도있다고 합니다.
    각 사건의 최신 점수, 중요도 점수, 유사도 점수를 가중합하여 높은 순으로 n_count개 만큼 저장합니다.
    '''
    gw = [0.5, 3, 2]
    master_out = dict()
